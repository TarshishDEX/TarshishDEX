...
export default function OrdersError({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    console.error("[orders] Unhandled route error:", error);
  }, [error]);

  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-4 text-center">
      <h1 className="font-display text-2xl font-semibold">Orders failed to load</h1>
      <p className="text-foreground-muted max-w-md text-sm">
        Could not load the orders page. The limit-order contract or RPC may be unavailable.
      </p>
