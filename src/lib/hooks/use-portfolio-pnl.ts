...
export function usePortfolioPnL(balances: AccountBalance[]) {
  ...
  const pnlByAsset = useMemo(() => {
    return balances.map((b) => {
      const key = b.token.issuer
        ? `${b.token.code}:${b.token.issuer}`
        : b.token.code;
      const cost = costBasis[key];
      const currentValue = b.valueInXlm ?? 0;
      const pnl = cost
        ? currentValue - (cost.totalCostXlm / cost.totalAmount) * b.balance
        : null;
  ...
  function setCostBasis(code: string, issuer: string | undefined, totalCostXlm: number, totalAmount: number) {
