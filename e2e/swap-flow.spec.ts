...
test.describe("Swap flow", () => {
  ...
  test("renders swap widget with form controls", async ({ page }) => {
    ...
    await expect(page.getByLabelText("Amount to pay")).toBeVisible();
    await expect(page.getByLabelText("Amount to receive")).toBeVisible();
    await expect(page.getByText("Max slippage")).toBeVisible();
  });

  test("shows Connect Wallet button when disconnected", async ({ page }) => {
    await expect(
      page.getByRole("button", { name: "Connect Wallet to Swap" })
    ).toBeVisible();
  });

  test("typing an amount enables interaction and fetches a quote", async ({ page }) => {
    const amountInput = page.getByLabelText("Amount to pay");
  ...
  test("reverses swap direction via arrow button", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Reverse swap direction" })
    ).toBeVisible();
  });
  ...
  test("switches to custom slippage input", async ({ page }) => {
    ...
    await expect(page.getByLabelText("Custom slippage percentage")).toBeVisible();
    await page.getByLabelText("Custom slippage percentage").fill("2.5");
    await expect(page.getByLabelText("Custom slippage percentage")).toHaveValue("2.5");
