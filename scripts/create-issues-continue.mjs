...
function issue(title, labels, body) {
  ...
  try {
    execSync(`gh issue create --repo "${REPO}" --title "${title.replace(/"/g, '\\"')}" --label "${labels}" --body-file "${file}"`, { stdio: "pipe" });
    console.log(`  [${count}/100] ${title}`);
  } catch (e) {
    console.error(`  [${count}/100] FAILED: ${title} - ${e.stderr?.toString()?.trim() || e.message}`);
